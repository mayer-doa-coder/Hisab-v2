...
- [ ] Events validated **structurally and for authenticity** before commit, using the *same* Zod schemas the client writes with (`packages/domain/src/events.ts`) — never a second, parallel validation scheme (`AGENTS.md` §4.3). That means: schema match, `schema_version` present, envelope well-formed (`id` a UUID, `seq` monotonic per `device_id`, `hlc` parseable), `shop_id` matching the authenticated shop, and every `_poisha` field a positive integer below an absurd-value ceiling.
- [ ] **The server never rejects an event for conflicting with derived state.** A payment exceeding the outstanding balance and a stock movement taking quantity below zero are both *valid events* that produce an anomaly after the fold (`docs/EVENTS.md` §8), surfaced to the shopkeeper — never blocked at write and never blocked at sync. The rule that makes this checkable: **the server may reject an event that could never be valid in any state; it may not reject one that is invalid only relative to a state it does not authoritatively hold.** `amount_poisha > 0` is the first kind. Balance-exceeded is the second. This bullet previously said the opposite — see `docs/DECISIONS.md` 2026-08-15 for why that was wrong.
- [ ] **No projection tables on the server.** It does not need `balances` to serve `GET /v1/events?since=`. With no server-side balance to check against, the bullet above is enforced by the schema rather than by anyone remembering it.
